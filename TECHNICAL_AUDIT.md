...
- **Second gap (still open, data layer not UI)**: `_syncStatus` is only mutated by `pushItemsToFirebase`, `triggerFullSync`, `syncOnAppOpen`, and a listener failure. The other six push functions (links, tasks, collections, collection items, todos, task types) never touch it, so a reactive sync of anything but items still shows nothing — now on five screens rather than two. Fixing this means touching `FirebaseSyncRepository`, not the UI.

### 5b. Per-tab Help Buttons Outrun the Manual (Content Gap)
- **Status**: ⚠️ Wiring complete, content missing for 4 of 5 tabs
- Every tab's `InventoriaTopBar` now shows a "?" aimed at that tab's `HelpCatalog` category, via `Screen.helpCategoryId`. Of the five, only **Task Tracker** (`"tasks"`) has articles — `today`, `todos`, `inventory` and `settings` are declared in `StubCategories.kt` with no `articles` at all.
...
- **Mitigation, not a fix**: `InventoriaApp.openHelpFor()` checks `articles.isNotEmpty()` and falls back to the manual's index. So four of the five help buttons currently open the index rather than their own section. They upgrade themselves with no code change as articles get written — the id is already correct.
