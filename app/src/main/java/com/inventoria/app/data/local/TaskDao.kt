...
interface TaskDao {
    ...
    @Query("SELECT * FROM tasks WHERE isRunning = 1 AND isDeleted = 0")
    fun getRunningTasks(): Flow<List<Task>>

    @Query("SELECT * FROM tasks WHERE isRunning = 0 AND isDeleted = 0 ORDER BY endTime DESC")
    fun getCompletedTasks(): Flow<List<Task>>

    @Query("SELECT * FROM tasks WHERE groupId = :groupId AND isDeleted = 0")
    ...
    @Query("UPDATE tasks SET isDeleted = 1, updatedAt = :timestamp WHERE groupId = :groupId")
    ...
    @Query("DELETE FROM tasks WHERE isDeleted = 1 AND updatedAt < :threshold")
    ...
    @Query("UPDATE tasks SET name = :newName, updatedAt = :timestamp WHERE groupId = :groupId AND isNameCustom = 0 AND isDeleted = 0")
    suspend fun updateSessionName(groupId: String, newName: String, timestamp: Long)

    @Query("UPDATE tasks SET kind = :newKind, updatedAt = :timestamp WHERE groupId = :groupId AND isKindCustom = 0 AND isDeleted = 0")
    suspend fun updateSessionKind(groupId: String, newKind: TaskKind, timestamp: Long)
    
    @Query("UPDATE tasks SET isSessionActive = 0, isRunning = 0, isPaused = 0, updatedAt = :timestamp WHERE groupId = :groupId AND isDeleted = 0")
    suspend fun endSession(groupId: String, timestamp: Long)

    @Query("UPDATE tasks SET isRunning = 0, isPaused = 0, isSessionActive = 0, endTime = :endTime, duration = :duration, updatedAt = :timestamp WHERE id = :taskId AND isDeleted = 0")
