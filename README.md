...
*   **Todo Tasks**: A separate "Todos" tab for real to-do items, distinct from time-tracked sessions. Each todo has a title, an optional multi-line **description** (shown clipped to two lines on the row), a Kind, an optional Task Type and an optional deadline — a date, plus an optional time of day that shows as "Due HH:MM" on the row and turns bold red once that minute passes on the day it's due. The time row is always offered; picking a time with no date fills the date in as today. The date is kept separate from the time internally so day-grouping and the overdue penalty keep working in whole days. An incomplete todo whose deadline has passed automatically carries over into Today's list (same item, no duplication) with an escalating "Overdue by N days" penalty subtracted from that day's score, capped at 5 points/day. Each day section shows an "X% Done" progress bar for whatever was actually due that day. A Start button on any todo kicks off a real tracked session seeded with its title, Kind and Type, and a pop-up confirms it with a shortcut to the tracker; when that session stops, a "Complete or still ongoing?" prompt ties the result back to the todo.
    ...
    *   **Repeating**: A todo with a deadline can repeat Daily, Weekly or Monthly. Once its deadline day passes it goes back to Incomplete and the deadline moves on one interval (its In Progress/Complete sub-todos reset with it). Each finished cycle is tallied on the todo -- completed if it was Complete when the cycle ended, missed otherwise, and every whole cycle skipped while the app was closed counts as missed. Settled by `TodoCycleRoller` (on launch, then once a minute) through the pure `Todo.settleCycles`.
*   **Schedule**: The Schedule segment of the Todos tab is a week strip over a 24-hour day timeline. **Schedule blocks** ("06:00–07:00 Gym") are designated time: a synced entity with a title, Kind colour, optional Task Type, optional daily or weekly repeat and notes. They are painted flat and edge to edge as the plan; tracked task segments are drawn as solid cards in front of them, lane-packed, stopping short of a strip down the right edge that always shows the block colour underneath, so plan and reality sit side by side. Todos due at a time appear as hairlines, all-day todos in a row above. Tapping empty space adds a block at that hour. A block never scores and never starts anything by itself; the Now card on Today can start a session from it, and that session inherits the block's Task Type so planned hours and tracked hours count under the same activity.
...
*   **Collection Presets**: Labels like TRAVEL_KIT, OUTFIT and WORK_GEAR, chosen when creating a collection. Purely descriptive — no behaviour is attached to the choice, and neither is `requiresSameLocation` (the "same location" checkbox on the same form): both are saved but nothing reads either back.
...
*   **In-App How To**: The **?** on every tab (and Settings → Help) opens a browsable manual — one category per area of the app, articles with numbered steps, a drawn diagram per step and a closing note on why each feature behaves as it does, plus a search box. All five tabs are documented in full: Today, Task Tracking, Todos (including the Schedule segment), Inventory & Items, and Settings. Collections & Readiness (reached from Inventory) is documented too. The remaining areas — Map, Productivity, Task Types and Sync & Accounts, each reached from within a tab rather than being a tab itself — are listed but not yet written, and open the index until they are. Features that exist in name only get explicit "Coming soon" entries rather than instructions for something that doesn't work.
...
*   **Write the remaining How To articles**: All five tabs are complete (Today, Task Tracking, Todos, Inventory & Items, Settings), and so is Collections & Readiness. Map, Productivity, Task Types and Sync & Accounts — each its own area reached from within a tab — are listed in the in-app guide but not yet written.
