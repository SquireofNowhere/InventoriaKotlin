...
*   **In-App How To**: The **?** on every tab (and Settings → Help) opens a browsable manual — one category per area of the app, articles with numbered steps, a drawn diagram per step and a closing note on why each feature behaves as it does, plus a search box. Today, Task Tracking, Todos (including the Schedule segment) and Inventory & Items are documented in full; the remaining areas are listed but not yet written, and their tab buttons open the index until they are. Features that exist in name only get explicit "Coming soon" entries rather than instructions for something that doesn't work.
...
*   **Write the remaining How To articles**: Today, Task Tracking, Todos and Inventory & Items are complete; Collections, Map, Productivity, Task Types, Sync and Settings are listed in the in-app guide but not yet written. Settings is next in priority order.
