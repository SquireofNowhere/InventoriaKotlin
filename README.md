...
*   **In-App How To**: The **?** on every tab (and Settings → Help) opens a browsable manual — one category per area of the app, articles with numbered steps, a drawn diagram per step and a closing note on why each feature behaves as it does, plus a search box. All five tabs are documented in full: Today, Task Tracking, Todos (including the Schedule segment), Inventory & Items, and Settings. The remaining areas — Collections, Map, Productivity, Task Types and Sync & Accounts, each reached from within a tab rather than being a tab itself — are listed but not yet written, and open the index until they are. Features that exist in name only get explicit "Coming soon" entries rather than instructions for something that doesn't work.
...
*   **Write the remaining How To articles**: All five tabs are complete (Today, Task Tracking, Todos, Inventory & Items, Settings). Collections, Map, Productivity, Task Types and Sync & Accounts — each its own area reached from within a tab — are listed in the in-app guide but not yet written.
