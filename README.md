...
*   **Todo Tasks**: A separate "Todos" tab for real to-do items, distinct from time-tracked sessions. Each todo has an optional date-only deadline; an incomplete todo whose deadline has passed automatically carries over into Today's list (same item, no duplication) with an escalating "Overdue by N days" penalty subtracted from that day's score, capped at 5 points/day. Todos support unlimited-depth parent/child nesting (GitHub-Projects-style sub-todos with indentation and a "X/Y sub-todos complete" progress readout on the parent — completing every child doesn't auto-complete the parent). Each day section shows an "X% Done" progress bar for whatever was actually due that day. A Start button on any todo kicks off a real tracked session seeded with its title and Kind; when that session stops, a "Complete or still ongoing?" prompt ties the result back to the todo.
